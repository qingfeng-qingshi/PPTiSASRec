...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen, relation_matrix, result_queue, SEED):
    """
    生成训练样本的批次。每个样本包括用户交互序列、时间序列、正样本和负样本以及相应的时间矩阵
    :param user_train:
    :param usernum:
    :param itemnum:
    :param batch_size:
...
def timeSlice(time_set):
    """
    将 time_set 中的每个时间戳映射为从零开始的整数，表示相对时间
    :param time_set:
    :return:
    """
...
def cleanAndsort(User, time_map):
    """
    清理并排序用户交互数据。将用户和物品映射为连续的整数，并规范化时间戳
    :param User:
    :param time_map:
    :return:
    """
...
def evaluate(model, dataset, args):
    ...
    for u in users:
        ...
        if rank < 10:
            ...
            HT += 1
            # 获取预测结果中排名前10的物品
            top_10_indices = predictions.argsort()[-10:]
            top_10_items = [item_idx[i] for i in top_10_indices]
            # 将 test[u] 转换为一个包含所有项目的集合
            test_set = set(item[0] for item in test[u])
            # 计算 Precision@10 和 Recall@10
            hits = len(set(top_10_items).intersection(set(test_set)))
            Precision += hits / 10.0
            Recall += hits / len(test[u])
        if valid_user % 100 == 0:
            print('.', end='')
            sys.stdout.flush()

    return NDCG / valid_user, HT / valid_user,Precision / valid_user, Recall / valid_user
